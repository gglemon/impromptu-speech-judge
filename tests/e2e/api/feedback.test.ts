...
import * as handler from "@/app/api/feedback/route";
...
describe("POST /api/feedback", () => {
  it("returns feedback on success", async () => {
    vi.mocked(callLLM).mockResolvedValueOnce(llmJson({ score: 7, summary: "Good job." }));
      ...
      test: async ({ fetch }) => {
        const res = await fetch({
          ...
          body: JSON.stringify({
            ...
            transcript: "Climate change is serious.",
            difficulty: "medium",
          }),
