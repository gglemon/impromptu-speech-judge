...
import { sparUserRebuttalHintPrompt } from "@/lib/prompts/spar";

export async function POST(req: NextRequest) {
  try {
    const { resolution, userSide, aiConstructive, userConstructive, userCrossfire, difficulty = "medium" } = await req.json();

    const prompt = sparUserRebuttalHintPrompt({ resolution, userSide, aiConstructive, userConstructive, userCrossfire, difficulty });
